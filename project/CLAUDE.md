...
issue edit with optimistic locking (D129), the fixed issue-link catalog (D17), and simple cloning (D60)
are done; re-typing/re-parenting an issue, watchers, voting, bulk actions, and the rank/renumber
...
- Fixed Epic/Sub-task hierarchy enforcement on issue creation, the fixed workflow's hardcoded
  ...
  locking, the fixed issue-link catalog, and simple cloning (done); the rest of Phase 3 is not.

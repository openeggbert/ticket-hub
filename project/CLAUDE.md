...
hardcoded transition rules (resolution required/cleared, sub-task completion gate), and full-replacement
issue edit with optimistic locking (D129) are done; re-typing/re-parenting an issue, cloning, issue
links, watchers, voting, bulk actions, and the rank/renumber migration are not — see `NEXT.md` for the
exact remaining list. All three phases share one open item: the Crow-based `ticket-hub` server target
has never been compiled in this environment (network access to `github.com` is blocked);
...
- Fixed Epic/Sub-task hierarchy enforcement on issue creation, the fixed workflow's hardcoded
  resolution/sub-task-completion rules on status changes, and full-replacement issue edit with
  optimistic locking (done); the rest of Phase 3 is not.
